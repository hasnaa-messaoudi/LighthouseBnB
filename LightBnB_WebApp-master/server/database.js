...
*/
  ...
  if (Object.keys(options).length) {
    ...
    if (options.city) {
      ...
      array.push(`city LIKE $${queryParams.length}`);
    }

    if (options.minimum_rating) {
      queryParams.push(parseInt(options.minimum_rating));
      array.push(`property_reviews.rating >= $${queryParams.length}`);
    }
    
    if (options.owner_id) {
      queryParams.push(parseInt(options.owner_id));
      array.push(`properties.owner_id = $${queryParams.length}`);
    }

    if (options.minimum_price_per_night) {
      queryParams.push(parseInt(options.minimum_price_per_night) * 100);
      array.push(`properties.cost_per_night >= $${queryParams.length}`);
    }
